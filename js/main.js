...
function init() {
    ...
    player.y = canvas.height - 110; // Выравнивание игрока под пропорции экрана
...
function toggleMusic(val) { gameState.musicEnabled = val; }
function toggleSFX(val) { gameState.sfxEnabled = val; }

function toggleControlStyle() {
    let ctrl = document.getElementById('touch-controls');
    if (ctrl.style.justifyContent === 'flex-start') {
        ctrl.style.justifyContent = 'space-between';
        document.getElementById('ctrl-style-btn').innerText = "СЛЕВА / СПРАВА";
    } else {
        ctrl.style.justifyContent = 'flex-start';
        ctrl.style.gap = '20px';
        document.getElementById('ctrl-style-btn').innerText = "ВСЁ СЛЕВА";
    }
}
