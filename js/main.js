...
// Динамическая подгонка Canvas под РЕАЛЬНЫЙ размер экрана устройства
function resizeCanvas() {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    if (player) {
        player.y = getGroundY(canvas) - 30; // Человечек всегда стои́т строго на земле
    }
}
...
function update() {
    ...
    player.update(dir);
    player.y = getGroundY(canvas) - 30; // Фиксируем высоту ног игрока
